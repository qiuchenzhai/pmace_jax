...
def build_parser():
    ...
    parser.add_argument('config_dir', type=str, help='Path to config file.', nargs='?', 
                        const='noiseless_data_reconstruction.yaml',
                        default='config/noiseless_data_reconstruction.yaml')
    return parser
...
def main():
    ...
    ref_mode_0_dir = os.path.join(data_dir, config['data']['ref_mode_0_dir'])
    ref_mode_1_dir = os.path.join(data_dir, config['data']['ref_mode_1_dir'])
    ...
    ref_probe_0 = load_img(ref_mode_0_dir)
    ref_probe_1 = load_img(ref_mode_1_dir)
    ...
    plot_FakeIC_img(init_obj, ref_img=ref_object, display_win=recon_win, save_dir=save_dir + 'init_object.png')
    plot_mode_0(init_probe, ref_img=ref_probe_0, save_dir=save_dir + 'init_probe.png')
    ...
    recon_args = dict(init_object=init_obj, init_probe=init_probe, recon_win=recon_win, 
                      ref_object=ref_object, ref_probe=[ref_probe_0, ref_probe_1],
                      num_iter=num_iter, joint_recon=joint_recon)
    # sanity_check_args = dict(init_object=ref_object, init_probe=[ref_probe_1, ref_probe_0], recon_win=recon_win, 
    #                          ref_object=ref_object, ref_probe=[ref_probe_1, ref_probe_0],
    #                          num_iter=num_iter, joint_recon=joint_recon)
    ...
    iter_add_mode = config['PMACE']['iter_add_mode']
    energy_ratio = config['PMACE']['energy_ratio']
    additional_mode_prop_dist = float(config['PMACE']['propagation_distance'])
    iter_orthogonalize_modes = config['PMACE']['iter_orthogonalize_modes']
    ...
    pmace_obj = PMACE(pmace_recon, y_meas, patch_crds, **recon_args,
                      ...
                      iter_add_mode=iter_add_mode, energy_ratio=energy_ratio,
                      fres_prop=True, dx=dx, wavelength=source_wavelength, propagation_dist=additional_mode_prop_dist,
                      iter_orthogonalize_modes=iter_orthogonalize_modes)

    pmace_result = pmace_obj()
    plot_FakeIC_img(pmace_result['object'], ref_img=ref_object, display_win=recon_win, save_dir=pmace_dir + 'recon_object.png')
    plot_mode_0(pmace_result['probe'][0], ref_img=ref_probe_0, save_dir=pmace_dir + 'recon_probe_mode_0.png')
    plot_mode_1(pmace_result['probe'][1], ref_img=ref_probe_1, save_dir=pmace_dir + 'recon_probe_mode_1.png')
